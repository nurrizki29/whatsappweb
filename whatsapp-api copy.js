...
const queryMysql = (penerima, msgbody) => {
  return new Promise((resolve, reject) => {
    ...
    db_portald3pajak.query(sql, function (err, result) {
      ...
      console.log("Whatsapp API -", "Row Inserted");
    });
    msgbody +=
...
const createSessionsFileIfNotExists = function () {
  if (!fs.existsSync(SESSIONS_FILE)) {
    try {
      ...
      console.log("Whatsapp API -", "Sessions file created successfully.");
    } catch (err) {
      console.log("Whatsapp API -", "Failed to create sessions file: ", err);
    }
  }
};
...
const createSession = function (id, description) {
  console.log("Whatsapp API -", "Creating session: " + id);
  ...
  client.on("qr", (qr) => {
    console.log("Whatsapp API -", "QR RECEIVED FOR ID:", id, qr);
  ...
  client.on("ready", () => {
    ...
    console.log("Whatsapp API -", `Client ${id} is ready!`);
    ...
    db_wa.query(sql, function (err, result) {
      ...
      result.forEach((data) => {
        ...
        db_wa.query(sql, function (err, result) {
          if (err) throw err;
          console.log("Whatsapp API -", "Log message updated on id=" + data.id);
        });
      });
    });
  });
  ...
  client.on("authenticated", () => {
    console.log("Whatsapp API -", `Client ${id} is authenticated!`);
  ...
  client.on("change_state", async (state) => {
    console.log("Whatsapp API -", id, " State :", state);
  });
  ...
  client.on("message", async (msg) => {
    ...
    console.log("Whatsapp API -", msg.type, msg.body);
          ...
          db_d3pjk.query(sql, async function (err, result) {
            ...
            for (let data of result) {
              ...
              if (!isRegisteredNumber) {
                ...
                console.log(
                  "Whatsapp API -",
                  "GAGAL " + data.npm + " 0" + data.whatsapp
                );
              } else {
                console.log(
                  "Whatsapp API -",
                  "SUKSES " + data.npm + " 0" + data.whatsapp
                );
                verified++;
              }
              await sleep(mls);
        ...
        msg.downloadMedia().then((media) => {
          ...
          if (media) {
            ...
            axios
              ...
              .catch(function (err) {
                console.log("Whatsapp API -", "Failed to save the file:", err);
...
waSocket.on("connection", function (socket) {
  ...
  socket.on("create-session", function (data) {
    console.log("Whatsapp API -", "Create session: " + data.id);
    createSession(data.id, data.description);
  });
  socket.on("remove-session", function (data) {
    console.log("Whatsapp API -", "Request remove session: " + data.id);
...
function whatsappPOSThandler(req, res) {
  ...
  async function postSendMessage(req, res) {
    ...
    if (sessions.length > 0) {
      ...
      let status_wa = await client.getState();
      console.log("Whatsapp API -", "STATUS", status_wa);
      if (status_wa !== "CONNECTED") {
        res.status(200).json({
          status: status_msg,
        });
